/**
 * MemoryManager — semantic memory for a project, backed by the MemPalace CLI.
 *
 * CLI-only (no MCP): the harness keeps a per-project palace under the active
 * project folder, points every agent's `MEMPALACE_PALACE_PATH` at it, and mines each agent's
...
export class MemoryManager {
  ...
  mineNow(): void {
    ...
    // `home` is the active project folder (the data root) — agents live directly under it.
