    ...
    try {
      ...
      const proc = pty.spawn(resolved, opts.args ?? [], {
        ...
        env: {
          ...
          FORCE_COLOR: '1',
          // Per-agent hive identity (AGENT_ID, PROJECT_ROOT, …) when provided.
          ...(opts.env ?? {})
        } as Record<string, string>
      });

      const session: PtySession = { id: opts.id, proc, cwd: opts.cwd, command: resolved };
      ...
      proc.onExit(({ exitCode, signal }) => {
        ...
        // A killed PTY's onExit can fire LATE — after a new session has already
        // reused this id (agents respawn under a constant id, e.g. 'pty-god').
        // If we've been superseded, this stale callback must NOT delete the live
        // replacement from the map (which would orphan it: every pty:write would
        // then fail) or run teardown (which would archive the live agent). Only
        // act when the session under this id is still THIS proc.
