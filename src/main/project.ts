...
export class ProjectManager {
  ...
  private injectedPrompt(meta: AgentMeta, dir: string, root: string, semanticMemory: boolean, knowledge: boolean): string {
    ...
    // Every turn's natural-language prose (the text you write outside tool calls) is
    // captured on Stop and published to the shared, human-visible Messages/"says" feed —
    // it is NOT a private scratchpad. So agents stay silent through intermediate steps and
    // speak only when they have something worth surfacing. Keeps the feed signal, not noise.
    const saysLine = 'SHARED FEED DISCIPLINE: the natural-language prose you write outside of tool calls is published to a shared activity feed the human reads — it is not a private scratchpad, so do not think out loud in it. Do NOT narrate intermediate steps ("let me check…", "now I\'ll…", "found it…", "reading the file…", "inbox clear…", "okay, next…"). Stay silent and let your tool calls do the work until you have something worth the human\'s attention: a conclusion, a result, a decision, a question, or a blocker. When you do speak, make it one self-contained takeaway. Think privately; publish sparingly.';
    return [
