  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      const addCharacter = async (agent: Agent) => {
        ...
        // Light the selection halo if this agent is the live selection — covers
        // the initially-selected agent at mount and any agent that spawns while
        // already selected (read live, not a captured value).
        if (agent.id === useStore.getState().selectedId) character.setSelected(true);
      ...
      const applyState = (agent: Agent, rt: Runtime, force = false) => {
        ...
        switch (agent.status) {
          ...
          case 'thinking':
            ...
            break;
          case 'waiting':
            // Parked at the desk awaiting god / another agent, NOT at the door
            // (that's reserved for agents that need the human).
            c.setStatusGlyph('none');
            c.sitAtDesk();
      ...
      const unsubscribe = useStore.subscribe((s, prev) => {
        ...
        if (s.selectedId !== lastSelected) {
          ...
          // one, and nudge the camera. An agent not yet spawned is lit later by
          // addCharacter's live-selectedId check.
          const prevRt = lastSelected ? runtimes.get(lastSelected) : undefined;
          prevRt?.character.setSelected(false);
          lastSelected = s.selectedId;
          const rt = s.selectedId ? runtimes.get(s.selectedId) : undefined;
          if (rt) {
            rt.character.setSelected(true);
      ...
      const offMessage = window.cth.onProjectMessage
        ? window.cth.onProjectMessage((e) => {
