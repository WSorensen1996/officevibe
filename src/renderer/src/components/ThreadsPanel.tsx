 ...
 * inline (sent as the "human" sender via window.cth.projectSend).
 */
...
function groupThreads(msgs: ProjectMessage[]): Thread[] {
  const by = new Map<string, ProjectMessage[]>();
  for (const m of msgs) {
...
export function ThreadsPanel({ agentId }: ThreadsPanelProps) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const inbox = await window.cth.projectInbox(agentId);
  ...
  const sendReply = async (last: ProjectMessage) => {
