 ...
 *
 *  All data comes from the existing preload bridge: store.agents + projectLog +
 *  projectMemory. No new IPC. Click an agent to jump to its memory; hover to peek. */
export function MemoryGraphPanel({
  ...
  const refresh = useCallback(async () => {
    try { setLog((await window.cth.projectLog(200)) as MessageLogEntry[]); } catch { /* noop */ }
  }, []);
  useEffect(() => {
    refresh();
  ...
  const fetchMemory = useCallback(async (id: string) => {
    try {
      const text = await window.cth.projectMemory(id);
  ...
  useEffect(() => {
    ...
    Promise.all(missing.map((id) => window.cth.projectMemory(id).then(
      (t) => [id, t ?? ''] as const,
      () => [id, ''] as const
    ))).then((pairs) => {
    ...
    <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', background: 'var(--cth-paper-200)' }}>
      ...
      <div ref={wrapRef} style={{ position: 'relative', flex: 1, minHeight: 0, overflow: 'hidden' }}>
        ...
        {messageEdgeCount === 0 && !showTopics && (
          ...
          }}>No messages logged yet — the project is quiet. Agents shown as roster.</div>
        )}
